...
class AmplitudeRabiF0g1Program(RAveragerProgram):
    ...
    def initialize(self):
        ...
        if cfg.expt.f0g1_cavity > 0:
            ...
            self.f0g1 = self.freq2reg(cfg.device.QM.pulses.f0g1.freq[cfg.expt.f0g1_cavity-1], gen_ch=self.qubit_chs[qTest])
            #self.f0g1_length = self.us2cycles(cfg.device.QM.pulses.f0g1.length[cfg.expt.f0g1_cavity-1], gen_ch=self.qubit_chs[qTest])
            self.f0g1_sigma = self.us2cycles(cfg.device.QM.pulses.f0g1.sigma[cfg.expt.f0g1_cavity-1], gen_ch=self.qubit_chs[qTest])
            self.add_gauss(ch=self.qubit_chs[qTest], name="f0g1", sigma=self.f0g1_sigma, length=self.f0g1_sigma*4)
    ...
    def body(self):
        ...
        if self.pulse_ge:
            ...
            self.sync_all(0.05)
        if self.pulse_ef:
            self.setup_and_pulse(ch=self.qubit_chs[qTest], style="arb", freq=self.f_ef, phase=0, gain=self.pief_gain, waveform="pief_qubit")
        ...
        self.set_pulse_registers(ch=self.qubit_chs[qTest],
                                  ...
                                  freq=self.f0g1, 
                                  phase=0, 
                                  gain=0, # placeholder; gain set by update
                                  waveform="f0g1")
