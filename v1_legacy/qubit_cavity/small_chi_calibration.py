...
class OutBackMeasurementProgram(AveragerProgram):
    def initialize(self):
    ...
    def body(self):
        cfg=AttrDict(self.cfg)
        qTest = self.qubits[0]
        self.N_placeholder = cfg.expt.displace_number
        self.tp = cfg.expt.tp
        self.displace_gain = cfg.expt.displace_gain

        if cfg.expt.pulse_ge:
        ...
        #  pulse the cavity and return with a swept phase offset, repeat for N times to amplify the phase
        for i in range(self.N_placeholder):
            self.setup_and_pulse(ch=self.man_chs[0], style="arb", freq=self.f_cavity, phase=self.deg2reg(0), gain=self.displace_gain, waveform="displace")
            self.sync_all(self.us2cycles(self.tp))
            self.setup_and_pulse(ch=self.man_chs[0], style="arb", freq=self.f_cavity, phase=self.deg2reg(self.cfg.expt.phase_placeholder), gain=self.displace_gain, waveform="displace")
            self.sync_all()
        ...
        self.measure(
            ...
            wait=True,
            syncdelay=self.us2cycles(self.cfg.expt.wait_time_r)
        )
        self.sync_all(self.us2cycles(0.05))
        # narrow bandwidth flip pulse
        ...
        self.setup_and_pulse(ch=self.qubit_chs[qTest], style="arb", freq=self.f_ge_init_reg, phase=0, gain=self.gain_ge_init, waveform="pi_middle_qubit")
        self.sync_all(self.us2cycles(0.05))
        self.measure(
            pulse_ch=self.res_chs[qTest], 
            adcs=[self.adc_chs[qTest]],
...
class OutBackMeasurementExperiment(Experiment):
    """OutBack measurement
       Experimental Config
    ...
    def __init__(self, soccfg=None, path='', prefix='OutBackMeasurement', config_file=None, progress=None):
    ...
    def acquire(self, progress=False):
        ...
        phi_list = self.cfg.expt["phase_start"] + self.cfg.expt["phase_step"] * np.arange(self.cfg.expt["phase_expts"])

        data = {"xpts": [], "avgi": [], "avgq": [], "i0":[], "i1":[], "q0":[], "q1":[]}
        self.cfg.expt.readout_no_placeholder = 2
        for phase_value in tqdm(phi_list, disable=not progress):
            self.cfg.expt.phase_placeholder = phase_value
            rspec = OutBackMeasurementProgram(soccfg=self.soccfg, cfg=self.cfg)
            self.prog = rspec
