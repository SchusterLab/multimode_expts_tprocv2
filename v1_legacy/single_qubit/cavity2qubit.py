...
class Cavity2QubitSweepProgram(AveragerProgram):
    ...
    def initialize(self):
        ...
        self.pisigma_resolved = self.us2cycles(
            self.cfg.expt.qubit_resolved_pi[2], gen_ch=self.qubit_chs[qTest])  # default resolved pi value
    ...
    def body(self):
        ...
        self.sync_all()
        # align between previous drive and qubit drive (positive:qubit, negative: cavity)

        if self.cfg.expt.length_placeholder>2:          

            self.setup_and_pulse(ch=self.qubit_chs[qTest], style="const", freq=self.f_ge_resolved_int_reg, length=self.cfg.expt.length_placeholder,
                                 phase=0, gain=0)
        elif self.cfg.expt.length_placeholder<-2:
            self.setup_and_pulse(ch=self.man_ch[qTest], style="const", freq=self.f_ge_resolved_int_reg, length=-self.cfg.expt.length_placeholder,
                                 phase=0, gain=0)
            

        # Qubit pi/2 pulse, wait gap time , pi/2 pulse again
        self.setup_and_pulse(ch=self.qubit_chs[qTest], style="arb", freq=self.f_ge_resolved_int_reg,
                                 phase=0, gain=self.cfg.expt.qubit_resolved_pi[1], waveform="pi_qubit_resolved")
        
        self.setup_and_pulse(ch=self.qubit_chs[qTest], style="const", freq=self.f_ge_resolved_int_reg, 
                             length=self.us2cycles(self.cfg.expt.qubit_resolved_pi[3]),
                             phase=0, gain=0)        
        ...
        if cfg.expt.prepulse:
            for ii in range(len(cfg.expt.pre_sweep_pulse[0])):
                ...
                if cfg.expt.pre_sweep_pulse[4][ii] == 1:
                    ...
                    self.tempch = self.flux_storage_ch
...
class Cavity2QubitSweepExperiment(Experiment):
    """
    ...
    def __init__(self, soccfg=None, path='', prefix='Cavity_qubit_sweep', config_file=None, progress=None):
    ...
    def acquire(self, progress=False, debug=False):
        ...
        for length in tqdm(lengths, disable=not progress):
            ...
            lengthrabi = Cavity2QubitSweepProgram(
    ...
    def display(self, data=None, fit=True, **kwargs):
        ...
        gain = self.cfg.expt.qubit_resolved_pi[2] 
        plt.subplot(
            211, title=f"Qubit readout (Gain {gain})", ylabel="I [adc level]")
