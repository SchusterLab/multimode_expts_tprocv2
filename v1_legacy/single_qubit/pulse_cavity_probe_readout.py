...
class PulseProbeSpectroscopyProgram(RAveragerProgram):
    ...
    def initialize(self):
        ...
        self.q_rp=self.ch_page(self.man_ch) # get register page for man_ch
        self.r_freq=self.sreg(self.man_ch, "freq") # get frequency register for man_ch    
        ...
        # self.ramp = self.us2cycles(cfg.expt.sigma, gen_ch=self.man_ch)
        ...
        self.set_pulse_registers(ch=self.man_ch, style="const", freq=self.f_start, phase=0, gain=cfg.expt.gain, length=self.us2cycles(cfg.expt.length, gen_ch=self.man_ch))
        # self.set_pulse_registers(ch=self.man_ch, style="flat_top", freq=self.f_start, phase=0, gain=cfg.expt.gain, 
        #                     length=self.us2cycles(cfg.expt.length, gen_ch=self.man_ch), waveform="ramp")
    ...
    def body(self):
        ...
        if cfg.expt.qubit_drive:
            self.setup_and_pulse(ch=self.qubit_ch, style="arb", 
                                 freq=self.freq2reg(cfg.device.qubit.f_ge, gen_ch=self.qubit_ch), phase=0, 
        ...
        self.pulse(ch=self.man_ch) # play probe pulse
