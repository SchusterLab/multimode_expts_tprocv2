...
class PhaseSweepProgram(RAveragerProgram):
    ...
    def body(self):
        ...
        #self.custom_pulse(cfg, cfg.expt.init_hpi_pulse)
        self.setup_and_pulse(ch=self.qubit_chs[qTest], style="arb", freq=self.f_ge_reg, phase=0, gain=self.hpi_gain, waveform="hpi_qubit")
        #pi ef 
        self.setup_and_pulse(ch=self.qubit_chs[qTest], style="arb", freq=self.f_ef_reg, phase=0, gain=self.pief_gain, waveform="pief_qubit")
        ...
        ##pi ef 
        self.set_pulse_registers(ch=self.qubit_chs[qTest], style="arb", freq=self.f_ef_reg, phase=0, gain=self.pief_gain, waveform="pief_qubit")
        ...
        self.set_pulse_registers(ch=self.qubit_chs[qTest], style="arb", freq=self.f_ge_reg, 
                                 phase= self.deg2reg(cfg.expt.advance_qubit_phase, gen_ch = self.qubit_chs[qTest]),
                                   gain=self.hpi_gain, waveform="hpi_qubit")
        #self.safe_regwi(self.q_rps[qTest], self.r_phase, self.deg2reg(cfg.expt.advance_qubit_phase, gen_ch = self.qubit_chs[qTest])) # add offset
    ...
    def update(self):
        qTest = self.qubits[0]
        phase_step = self.deg2reg(self.cfg.expt.step) # phase step [deg]
        self.mathi(self.q_rps[qTest], self.r_phase2, self.r_phase2, '+', phase_step) 
        


class PhaseSweepExperiment(Experiment):
    """
    Ramsey experiment
    Experimental Config:
    expt = dict(
    ...
    def acquire(self, progress=False, debug=False):
        ...
        ramsey = PhaseSweepProgram(soccfg=self.soccfg, cfg=self.cfg)
