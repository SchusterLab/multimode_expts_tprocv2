...
"""
Sweeps flux and measures the resonator response.
...
"""
class FluxSweepProgram(AveragerProgram):
...
class FluxSweepExperiment(Experiment):
    """
    ...
    def acquire(self, progress=False):
        ...
        for v in tqdm(vpts, disable=not progress):
            qdac.ch17.dc_constant_V(v)
            for f in tqdm(xpts, disable=not progress):
                self.cfg.expt.frequency = f
                rspec = FluxSweepProgram(soccfg=self.soccfg, cfg=self.cfg)
                # print(rspec)
