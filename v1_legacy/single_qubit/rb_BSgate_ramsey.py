...
class SingleBeamSplitterRB_ramsey_prog(MMDualRailRAveragerProgram):
    """
    ...
    def create_variation(self, depth):
        ...
        rb_time, bs_gate_num = self.get_total_time_from_running_list( running_list=running_list,
    ...
    def collect_shots_rb(self, read_num, expts):
        ...
        cfg = self.cfg
        # print(self.di_buf[0])
        shots_i0 = self.di_buf[0].reshape((expts, read_num, self.cfg["reps"]),order='F') / self.readout_lengths_adc
        # print(shots_i0)
        shots_q0 = self.dq_buf[0].reshape((expts, read_num, self.cfg["reps"]),order='F') / self.readout_lengths_adc
...
# ===================================================================== #
# play the pulse
class SingleBeamSplitterRB_ramsey(Experiment):
    ...
    def acquire(self, progress=False, debug=False):
        ...
        for var in tqdm(range(self.cfg.expt.variations)):   # repeat each depth by variations
            ...
            self.prog = rb_shot
            x_pts, avgi, avgq = rb_shot.acquire(
